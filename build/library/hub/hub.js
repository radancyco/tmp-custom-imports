...
var hubFeature = {
    Init: function(hubID){
        ...
        $('.js-hub-load-more-button').on('click', function (e) {
            ...
            if ( ( maxLoad != 0 && currentLoad >= maxLoad ) ) { // Make sure maxLoad is not set to 0 and then check if current count is higher than max load or if maxload is not set to zero but loadMore is set to show all 
                overMax = 1;
                console.log("Hub ID:" + thisHub + "Message: Over Max")
            } else if ( maxLoad == 0 ) { // If maxLoad is set to zero then we will never hit maxload
                overMax = 0;
                console.log("Hub ID:" + thisHub + "Message: Not Over Max")
            }
    ...
    setupPreFilters: function(hubID,setupFilters){
        ...
        console.log("Hub ID:" + thisHub + "Message: Mappings Deleted");
        $(hubID + " .js-hub-content > li:not(.pre-filtered)").remove();
        
        if(setupFilters == 1){
            console.log("Hub ID:" + thisHub + "Message: FINISHED");
            hubFeature.setupFilters(hubID);
        }
    },
    ...
    function sort_li(a, b) {
        console.log("Hub ID:" + thisHub + "Message: sort LI ran");
    ...
    filterData: function(hubID){
        ...
        $(hubID + " .hub-filter div select").each(function(){
            ...
            if(curValue != "none"){
                // weighted search
                if(isWeighted == "1"){
                    console.log("Hub ID:" + thisHub + "Message: weighted running");
                    ...
                    console.log("Hub ID:" + thisHub + "Message: not-weighted running");
                    fieldsUsed = fieldsUsed + "[" + curField + "='" + curValue + "']";
                    $(hubID + " ul.mappings > li" + fieldsUsed).each(function(){
        ...
        // not weighted
        if(isWeighted != "1"){
            console.log("Hub ID:" + thisHub + "Message: fields matched " + fieldsUsed);
            $(hubID + " ul.mappings > li" + fieldsUsed).each(function(){
                $(this).parent().parent().addClass("showing-by-filter").removeClass("hidden-by-filter");  
            })
        } else{
    ...
    loadMoreReset: function(hubID){
        ...
        if ( thisHub.attr('data-load-more-current') != null ) { // if it is not null then that means it has run before and we should ue current count instead of default
            ...
            console.log("Hub ID:" + thisHub + "Message: Current Already Exists")
        } else {
            ...
            console.log("Hub ID:" + thisHub + "Message: Current  Does not Exist but is now set")
        }

        if ( setCount == 0 ) {
            thisHub.attr('data-load-more-current', maxLoad); // Set the current load ammount to maxload
            console.log("Hub ID:" + thisHub + "Message: Set count Updated")
        }
