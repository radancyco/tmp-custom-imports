...
> **⚠ UPDATE (AUG 2022):** Topics are being phased out and _ideally_ should no longer be used a strategy for new HUBs. In some cases, particularly when working on a legacy site type or a GST site that does not and will not use multi-lingual functionality, topics may be used if necessary. **Topics should never be used on a multi-lingual GST site, or on a GST site that may use multi-lingual functionality at some point.**

> **⚠ FOR GLOBAL SITE TYPES ONLY:** HUBs that live on a GST site are built with the Content Feed module instead of the Custom HTML module (detailed instructions can be found below.) Keep in mind that in its current form, the Content Feed module CANNOT be used as both a HUB _and_ as Related Content on the same site. Until further notice is not possible for GST sites to have both a HUB and Related Content. It should also be noted that if a GST site may at any point utilize multi-lingual functionality, topics cannot be used as a prefilter for HUBs. Please check with Brock if you have any questions regarding these limitations.
...
-   If using topics, these content pages should have topic keys and values mapped to them already (This is done by the DPM/AT/Client).
...
6. Now push your changes to your preview site and test to make sure the HUB is working.
    ...
    - If you see the message _No results found, please come back later_, this means that the content pages require mappings to meet the prefiltering requirements. Often the step of mapping content pages is missed. To verify that the HUB is working, you can remove the facet limit(s) in the Content Feed module's **Prefiltering** settings.
    - You should be able to click on each filter dropdown and see what is currently mapped to all content pages in the system.
...
$(document).on('hubInitialized', function (e) {
	console.log(
		'Initialized for hubID: ' +
			e.detail.hubID +
			' Number of HUBs to initialize: ' +
			e.detail.index +
			' All HUBs initialized: ' +
			e.detail.final
	);
});
```
